...
╰──────────────────────╯`
  ...
  if (mode === 'on') maintenance = true
  else if (mode === 'off') maintenance = false
  else if (!mode) {
    // Hanya tampilkan tombol, tidak mengubah status
    await sock.sendMessage(from, {
    ...
    // Jika bukan 'on', 'off', atau kosong → perintah tidak valid
    await sock.sendMessage(from, {
